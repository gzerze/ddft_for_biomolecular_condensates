...
Gamma = 1    # Short-range cutoff factor
intlim= 200
...
#def chi_calc_2(HP,u,phi):
#    ehs = HP['ehs']
#    return gv.r_res*( ehs[0]*u + ehs[1] )*phi*(1-gv.r_res*phi)
    
# free energy functions
def feng(HP, phi, phis, u):
    return Enp(HP, phi, phis) + fel(HP, phi,phis,u) +gv.r_res**(-1)* chi_calc(HP, u)*phi*(1-gv.r_res*phi)

def dfeng(HP, phi, phis, u):
    return dEnp(HP, phi, phis) + dfel(HP, phi,phis,u) + chi_calc(HP,u)*(-2*phi)
...
def Enp2(HP, phi, phis,phic): 
    phic = HP['pc']*phi + phis
    return 1/HP['N']*s_calc(phi) + s_calc(phic) + s_calc(phis) \
           + s_calc(1-phi*gv.r_res-phic*gv.r_con-phis*gv.r_sal)


def dfeng_phic(HP,phi,phis,u):
    return dfeng(HP,phi,phis,u)/HP['pc']



def chem_pot(HP,phi,phis,u):
    ri=1.0
    ti=1.0
    #return ri*(1-phi)*feng(HP,phi,phis,u)
    return ri*feng(HP,phi,phis,u) + ti*dfeng(HP,phi,phis,u)-ri*phi*dfeng(HP,phi,phis,u)
